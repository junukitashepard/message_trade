...
baseline_EMISS <- import_gdx_emissions(scenario = 'baseline', version = 13, scenario.name = 'Baseline')
  baseline_EMISS <- unique(baseline_EMISS[c('year', 'node', 'emissions_TCE')])
  names(baseline_EMISS) <- c('year', 'node', 'baseline_emissions')
  
tariff_high_EMISS <- import_gdx_emissions(scenario = 'tariff_high', version = 13, scenario.name = 'High tariff')
tariff_low_EMISS <- import_gdx_emissions(scenario = 'tariff_low', version = 7, scenario.name = 'Low tariff')

CO2_tax_baseline_EMISS <- import_gdx_emissions(scenario = 'CO2_tax_baseline', version = 23, scenario.name = 'CO2 tax, baseline') 
CO2_tax_tariff_high_EMISS <- import_gdx_emissions(scenario = 'CO2_tax_tariff_high', version = 8, scenario.name = 'CO2 tax, high tariff')  
CO2_tax_tariff_low_EMISS <- import_gdx_emissions(scenario = 'CO2_tax_tariff_low', version = 5, scenario.name = 'CO2 tax, low tariff')  

sanction_NAM_CPA_EMISS <- import_gdx_emissions(scenario = 'NAM_CPA_sanction', version = 9, scenario.name = 'Sanction: NAM-CPA')   
sanction_CPA_PAO_EMISS <- import_gdx_emissions(scenario = 'CPA_PAO_sanction', version = 7, scenario.name = 'Sanction: CPA-PAO')
sanction_NAM_MEA_EMISS <- import_gdx_emissions(scenario = 'NAM_MEA_sanction', version = 7, scenario.name = 'Sanction: NAM-MEA')
...
############################
plot_emissions <- function(indf, plot.region, global.title, include.legend = 'none', y_text = '', fontsize = 12) {
