...
compile_tables <- function (variable) {
  
  wb <- loadWorkbook(file.path(output, 'base_regression.xlsx'))

  # All regions
  all_regions <- run_reg(variable = variable)
  ...
  for (r in region_list) {
    ...
    assign(paste0('M_mat'), run_reg(variable = variable, importers = r))
  ...
  for (r in region_list) {
    ...
    assign(paste0('X_mat'), run_reg(variable = variable, exporters = r))
  ...
  # Save compiled table
  saveWorkbook(wb, file.path(output, paste0(variable, '_regressions.xlsx')), overwrite = T)
}

# Run program
compile_tables(variable = 'distance')