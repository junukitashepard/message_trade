...
trade <- subset(trade, var_cost < 100)
...
# Run regressions
# All regions
df <- run_reg(variable.rr = 'distance')
df <- as.data.frame(df)
df$region <- 'ALL'
...
for (r in region_list) {
  ...
  assign(paste0('mat'), run_reg(variable.rr = 'distance', exporters = r))
...
ggplot(aes(x = region_factor, y = coef, fill = region_factor), data = df) +
  ...
  labs(x = '', y = 'Effect of 1000km increase on trade cost ($/GWa)', fill = 'Exporting Region') + 
