...
add_scenario <- function(scenario_name, message_version) {
  ...
  # Assign objective cost
  obj_cost <- rgdx(file.path(msg_dir, paste0('MsgOutput_MESSAGE_TRADE__', scenario_name, '__v', message_version, '.gdx')),
                   list(name = 'OBJ'))['val']
  assign(paste0('objcost_', scenario_name), as.numeric(obj_cost), envir = parent.frame())
...
build_table <- function(table_year) {
  ...
  basemat <- matrix(nrow = 24, ncol = 6)
  colnames(basemat) <- scenario_list
  rownames(basemat) <- c('system_cost', 'N_exporter', 'N_importer', 'N_link', 'total_trade',
  ...
  for (scen in colnames(basemat)) {
    ...
    assign('obj_cost', get(paste0('objcost_', scen)))
    invec['system_cost'] <- obj_cost
